...
export default function DashboardPage() {
  ...
  async function init() {
    ...
    setUser(user);
    const { data } = await supabase.from('user_credentials').select('*').eq('user_id', user.id).single();
    if (!data || !data.yt_refresh_token || (!data.ai_api_key && !data.openrouter_api_key)) {
      setCreds(null); setCredsLoading(false); return;
    }
    setCreds(data); setCredsLoading(false);
  }

  async function handleFetch() {
    const id = extractVideoId(urlInput);
    if (!id) { showToast('⚠️ Video URL ya ID daalo!', 'warn'); return; }
    setFetching(true); setSelectedVideo(null); setTags('');
    try {
      const res  = await fetch(`/api/youtube?videoId=${id}`);
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(typeof data.error === 'string' ? data.error : JSON.stringify(data.error) || 'Fetch fail');
      setSelectedVideo(data); setTags((data.tags || []).join(', ')); setStatus('idle');
  ...
  async function generateTags() {
    ...
    try {
      const text = await aiCall(
        ...
        creds.ai_provider || 'openrouter', activeKey
      );
      setTags(text.trim());
  ...
  async function handleUpdate() {
    ...
    setSaving(true);
    try {
      const res  = await fetch('/api/youtube', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ videoId: selectedVideo.videoId, tags }) });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(typeof data.error === 'string' ? data.error : data.error?.message || JSON.stringify(data.error) || 'Update fail');
      setStatus('saved'); setSelectedVideo(prev => ({ ...prev, tags: tagList }));
      showToast('✅ Tags YouTube pe update ho gaye!', 'success');
      setTimeout(() => setStatus('idle'), 3000);
    } catch (e) { setStatus('error'); showToast('❌ ' + e.message, 'error'); setTimeout(() => setStatus('idle'), 3000); }
    setSaving(false);
  }
  ...
  if (!credsLoading && !creds) {
    return (
      <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', flexDirection: 'column' }}>
        ...
        {settingsOpen && <SettingsDrawer supabase={supabase} user={user} onClose={() => { setSettingsOpen(false); refreshCreds(); }} showToast={showToast} />}
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', flexDirection: 'column' }}>
      {toast.msg && (
        <div style={{ position: 'fixed', top: 16, left: '50%', transform: 'translateX(-50%)', background: tc.bg, border: `1px solid ${tc.border}`, borderRadius: 12, padding: '10px 18px', fontSize: 13, color: tc.color, zIndex: 999, boxShadow: '0 4px 20px rgba(0,0,0,0.6)', fontWeight: 600, maxWidth: '85vw', textAlign: 'center' }}>
          {toast.msg}
        </div>
      )}
      <Topbar user={user} onSettings={() => setSettingsOpen(true)} onLogout={handleLogout} showBack={!!selectedVideo} onBack={() => { setSelectedVideo(null); setTags(''); setUrlInput(''); setStatus('idle'); }} />
      <div style={{ flex: 1, padding: '14px 12px 40px', maxWidth: 600, margin: '0 auto', width: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {!selectedVideo && (
          <div style={{ background: '#0c0c0c', border: '1px solid #1e1400', borderRadius: 14, padding: 14 }}>
            ...
            <input value={urlInput} onChange={e => setUrlInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleFetch()} placeholder="YouTube URL ya Video ID daalo..." style={{ width: '100%', background: '#080808', border: '1px solid #252525', borderRadius: 10, padding: '11px 14px', fontSize: 13, color: '#eee', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit', marginBottom: 10 }} />
            <button onClick={handleFetch} disabled={fetching || !urlInput.trim()} style={{ width: '100%', padding: '13px', borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: fetching || !urlInput.trim() ? 'not-allowed' : 'pointer', background: fetching || !urlInput.trim() ? '#0a0a0a' : 'linear-gradient(135deg,#ff8c00,#ff4400)', border: fetching || !urlInput.trim() ? '1px solid #1a1a1a' : 'none', color: fetching || !urlInput.trim() ? '#333' : '#fff', boxShadow: fetching || !urlInput.trim() ? 'none' : '0 4px 20px rgba(255,140,0,0.3)' }}>
        ...
        {selectedVideo && (
          <>
            <div style={{ background: '#0c0c0c', border: '1px solid #1e1400', borderRadius: 14, overflow: 'hidden' }}>
              {selectedVideo.thumbnail && <img src={selectedVideo.thumbnail} alt="" style={{ width: '100%', maxHeight: 200, objectFit: 'cover' }} />}
            ...
            <div style={{ background: '#0c0c0c', border: `1px solid ${isDirty ? '#ff8c0033' : '#1a1a1a'}`, borderRadius: 14, padding: 14 }}>
              ...
              <textarea value={tags} onChange={e => { setTags(e.target.value); setStatus('idle'); }} placeholder="tag1, tag2, tag3... (comma separated)" rows={4} style={{ width: '100%', background: '#080808', border: `1px solid ${isDirty ? '#ff8c0033' : '#1e1e1e'}`, borderRadius: 10, padding: '10px 12px', fontSize: 12, color: '#eee', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit', resize: 'vertical', lineHeight: 1.6, marginBottom: 10 }} />
              {tagList.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
                  {tagList.map((t, i) => <span key={i} style={{ background: '#1a0e00', border: '1px solid #ff8c0022', color: '#ff8c0088', borderRadius: 20, padding: '2px 9px', fontSize: 10, fontWeight: 600 }}>{t}</span>)}
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={generateTags} disabled={generating} style={{ flex: 1, background: generating ? '#0a0a0a' : 'linear-gradient(135deg,#120a22,#0a0014)', border: `1px solid ${generating ? '#222' : '#8855cc44'}`, color: generating ? '#444' : '#aa77ee', borderRadius: 10, padding: '12px', fontSize: 12, fontWeight: 700, cursor: generating ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  {generating ? '⏳ Generating...' : '🤖 AI Generate'}
                </button>
                <button onClick={handleUpdate} disabled={saving || !isDirty} style={{ flex: 1, background: saving ? '#0a0a0a' : isDirty ? 'linear-gradient(135deg,#2a1000,#1a0500)' : '#0a0a0a', border: `1px solid ${saving ? '#222' : isDirty ? '#ff8c0055' : '#1a1a1a'}`, color: saving ? '#444' : isDirty ? '#ff8c00' : '#333', borderRadius: 10, padding: '12px', fontSize: 12, fontWeight: 700, cursor: saving || !isDirty ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  {saving ? '⏳ Updating...' : '🚀 Update'}
                </button>
              </div>
            </div>
            <button onClick={() => { setSelectedVideo(null); setTags(''); setUrlInput(''); setStatus('idle'); }} style={{ background: '#0c0c0c', border: '1px solid #1a1a1a', color: '#555', borderRadius: 10, padding: '11px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              🔁 Doosra Video Fetch Karo
            </button>
          </>
        )}
        {credsLoading && <div style={{ textAlign: 'center', padding: 40, color: '#333', fontSize: 12 }}>Loading...</div>}
      </div>
      {settingsOpen && <SettingsDrawer supabase={supabase} user={user} onClose={() => { setSettingsOpen(false); refreshCreds(); }} showToast={showToast} />}
      <style>{`
...
function SettingsDrawer({ supabase, user, onClose, showToast }) {
  ...
  const [form, setForm] = useState({ channel_id: '', yt_refresh_token: '', ai_provider: 'openrouter', groq_api_key: '', openrouter_api_key: '' });
  ...
  async function loadData() {
    const { data } = await supabase.from('user_credentials').select('*').eq('user_id', user.id).single();
    if (data) {
      setForm({ channel_id: data.channel_id || '', yt_refresh_token: data.yt_refresh_token || '', ai_provider: data.ai_provider || 'openrouter', groq_api_key: data.ai_api_key || '', openrouter_api_key: data.openrouter_api_key || '' });
      if (data.yt_refresh_token) {
        ...
        try {
          ...
          if (ytData.channelTitle) {
            setChannelInfo({ name: ytData.channelTitle, avatar: ytData.channelAvatar || '', subscribers: ytData.subscriberCount || '0', subscriberHidden: ytData.subscriberHidden || false });
          }
        } catch (_) {}
  ...
  async function handleProviderSwitch(provider) {
    ...
    providerSaveTimer.current = setTimeout(async () => {
      await supabase.from('user_credentials').upsert({ user_id: user.id, ai_provider: provider, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
  ...
  async function handleDisconnect() {
    await supabase.from('user_credentials').upsert({ user_id: user.id, yt_refresh_token: null, channel_id: null, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
  ...
  function handleConnectYouTube() {
    ...
    const scope = encodeURIComponent(['https://www.googleapis.com/auth/youtube', 'https://www.googleapis.com/auth/youtube.force-ssl'].join(' '));
    const clientId = process.env.NEXT_PUBLIC_YOUTUBE_CLIENT_ID;
    if (!clientId) { showToast('❌ Server config error', 'error'); return; }
    window.location.href = `https://accounts.google.com/o/oauth2/auth?client_id=${encodeURIComponent(clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${scope}&access_type=offline&prompt=consent`;
  }

  async function handleAddKey() {
    if (!newKey.trim()) { showToast('❌ API key daalo!', 'error'); return; }
    setAddingKey(true);
    const res = await fetch('/api/youtube', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'add', api_key: newKey.trim(), label: newLabel.trim() || `Key ${apiKeys.length + 1}` }) });
    const data = await res.json();
  ...
  async function handleDeleteKey(id) {
    ...
    showToast('🗑️ Key delete ho gayi', 'warn'); loadApiKeys();
  }
  ...
  async function handleReactivateKey(id) {
    ...
    showToast('✅ Key reactivate ho gayi!', 'success'); loadApiKeys();
  }
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, animation: 'fadeIn 0.2s ease' }} />
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 101, width: '88vw', maxWidth: 420, background: '#0a0a0a', borderLeft: '1px solid #1e1e1e', borderRadius: '20px 0 0 20px', overflowY: 'auto', animation: 'slideInRight 0.28s cubic-bezier(0.32,0.72,0,1)', paddingBottom: 32 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 12px', position: 'sticky', top: 0, background: '#0a0a0a', zIndex: 2, borderBottom: '1px solid #141414' }}>
          ...
          <button onClick={onClose} style={{ background: '#161616', border: '1px solid #2a2a2a', color: '#666', borderRadius: 8, width: 30, height: 30, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>✕</button>
        </div>

        <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>

          {/* User info with shimmer */}
          {channelLoading ? (
            ...
            <div style={{ background: '#0c0c0c', border: '1px solid #1a1a1a', borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: '#555', marginTop: 3 }}>
                  {ytConnected
                    ? channelInfo.subscriberHidden ? '🔒 Subscribers hidden' : `👥 ${formatSubscribers(channelInfo.subscribers)} subscribers`
                    : user?.email}
                </div>
              </div>
          ...
          <DrawerSection title="🎬 YouTube">
            {ytConnected ? (
              <>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={handleConnectYouTube} style={{ flex: 1, padding: '11px', borderRadius: 10, fontSize: 12, fontWeight: 800, cursor: 'pointer', background: '#080f08', border: '1px solid #44bb6622', color: '#44bb6677' }}>🔄 Re-connect</button>
                  <button onClick={handleDisconnect} style={{ flex: 1, padding: '11px', borderRadius: 10, fontSize: 12, fontWeight: 800, cursor: 'pointer', background: '#100000', border: '1px solid #ff000022', color: '#ff4444' }}>🔌 Disconnect</button>
                </div>
              </>
            ) : (
              <button onClick={handleConnectYouTube} style={{ width: '100%', padding: '13px', borderRadius: 12, fontSize: 13, fontWeight: 800, cursor: 'pointer', background: 'linear-gradient(135deg,#001a0a,#001408)', border: '1px solid #00cc6644', color: '#00cc66' }}>🔗 Connect YouTube</button>
            )}
          </DrawerSection>

          {/* AI Section — only active provider input */}
          <DrawerSection title="🤖 AI Settings">
            <div style={{ display: 'flex', gap: 8 }}>
              {['openrouter', 'groq'].map(p => (
                <button key={p} onClick={() => handleProviderSwitch(p)} style={{ flex: 1, padding: '10px', borderRadius: 10, fontSize: 12, fontWeight: 800, cursor: 'pointer', background: form.ai_provider === p ? (p === 'groq' ? '#001208' : '#120800') : '#0a0a0a', border: `1px solid ${form.ai_provider === p ? (p === 'groq' ? '#00cc6644' : '#ff8c0044') : '#1a1a1a'}`, color: form.ai_provider === p ? (p === 'groq' ? '#00cc66' : '#ff8c00') : '#444', transition: 'all 0.15s' }}>
            ...
            {form.ai_provider === 'openrouter' ? (
              <div>
                <div style={{ fontSize: 10, color: '#ff8c00', fontWeight: 800, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.8px' }}>🔶 OpenRouter API Key <span style={{ color: '#ff8c0055', fontWeight: 600, textTransform: 'none', letterSpacing: 0 }}>← active</span></div>
                <div style={{ position: 'relative' }}>
                  <input type={showOpenrouterKey ? 'text' : 'password'} value={form.openrouter_api_key} onChange={e => setForm(f => ({ ...f, openrouter_api_key: e.target.value }))} onBlur={() => handleKeyBlur('openrouter')} placeholder="sk-or-xxxxxxxxxxxx" style={{ ...inputStyle, paddingRight: 44, borderColor: '#ff8c0033' }} />
                  <button onClick={() => setShowOpenrouterKey(p => !p)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: 14, padding: 4 }}>{showOpenrouterKey ? '🙈' : '👁'}</button>
                </div>
              </div>
            ) : (
              <div>
                <div style={{ fontSize: 10, color: '#00cc66', fontWeight: 800, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.8px' }}>⚡ Groq API Key <span style={{ color: '#00cc6655', fontWeight: 600, textTransform: 'none', letterSpacing: 0 }}>← active</span></div>
                <div style={{ position: 'relative' }}>
                  <input type={showGroqKey ? 'text' : 'password'} value={form.groq_api_key} onChange={e => setForm(f => ({ ...f, groq_api_key: e.target.value }))} onBlur={() => handleKeyBlur('groq')} placeholder="gsk_xxxxxxxxxxxx" style={{ ...inputStyle, paddingRight: 44, borderColor: '#00cc6633' }} />
                  <button onClick={() => setShowGroqKey(p => !p)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: 14, padding: 4 }}>{showGroqKey ? '🙈' : '👁'}</button>
                </div>
              </div>
            )}
            ...
            <div style={{ background: '#080808', border: '1px solid #141414', borderRadius: 10, padding: '8px 12px', fontSize: 11, color: '#2a2a2a', lineHeight: 1.6 }}>
              Active model: <span style={{ color: form.ai_provider === 'groq' ? '#00cc6655' : '#ff8c0055' }}>{form.ai_provider === 'groq' ? 'llama-3.3-70b-versatile' : 'openai/gpt-4o-mini'}</span>
            </div>
          </DrawerSection>

          {/* YouTube API Keys — popup trigger */}
          <DrawerSection title="🔑 YouTube API Keys (Quota Rotation)">
            ...
            <button onClick={() => setShowApiKeysModal(true)} style={{ width: '100%', background: '#0a0a0a', border: '1px solid #ff8c0033', color: '#ff8c00', borderRadius: 10, padding: '11px', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              🔑 Manage API Keys
              <span style={{ background: '#1a0a00', border: '1px solid #ff8c0033', borderRadius: 20, padding: '1px 8px', fontSize: 10, color: '#ff8c0099' }}>{apiKeys.length} added</span>
            </button>
          </DrawerSection>
      ...
      {showApiKeysModal && (
        <>
          <div onClick={() => { setShowApiKeysModal(false); setShowAddKey(false); setNewKey(''); setNewLabel(''); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 200, animation: 'fadeIn 0.15s ease' }} />
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 201, background: '#0e0e0e', borderTop: '1px solid #222', borderRadius: '20px 20px 0 0', maxHeight: '75vh', display: 'flex', flexDirection: 'column', animation: 'slideUp 0.25s cubic-bezier(0.32,0.72,0,1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px', borderBottom: '1px solid #181818', flexShrink: 0 }}>
              ...
              <button onClick={() => { setShowApiKeysModal(false); setShowAddKey(false); setNewKey(''); setNewLabel(''); }} style={{ background: '#161616', border: '1px solid #2a2a2a', color: '#666', borderRadius: 8, width: 28, height: 28, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>✕</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {apiKeys.length === 0 && !showAddKey && <div style={{ textAlign: 'center', padding: '24px 0', color: '#333', fontSize: 12 }}>Koi key add nahi hui abhi</div>}
              {apiKeys.map(k => (
                <div key={k.id} style={{ background: '#0a0a0a', border: `1px solid ${k.is_active ? '#ff8c0022' : '#ff000022'}`, borderRadius: 12, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: k.is_active ? '#ff8c00' : '#ff4444', marginBottom: 3 }}>{k.is_active ? '🟢' : '🔴'} {k.label}</div>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    {!k.is_active && <button onClick={() => handleReactivateKey(k.id)} style={{ background: '#001a08', border: '1px solid #44bb6622', color: '#44bb66', borderRadius: 6, padding: '5px 10px', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>↺ Reset</button>}
                    <button onClick={() => handleDeleteKey(k.id)} style={{ background: '#100000', border: '1px solid #ff000033', color: '#ff4444', borderRadius: 6, padding: '5px 10px', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>🗑</button>
                  </div>
                </div>
              ))}
              {showAddKey && (
                <div style={{ background: '#0c0c0c', border: '1px solid #252525', borderRadius: 12, padding: 12, display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={handleAddKey} disabled={addingKey} style={{ flex: 1, background: addingKey ? '#0a0a0a' : 'linear-gradient(135deg,#ff8c00,#ff4400)', border: 'none', color: addingKey ? '#333' : '#fff', borderRadius: 8, padding: '10px', fontSize: 12, fontWeight: 700, cursor: addingKey ? 'not-allowed' : 'pointer' }}>{addingKey ? '⏳ Adding...' : '✅ Add Key'}</button>
                    <button onClick={() => { setShowAddKey(false); setNewKey(''); setNewLabel(''); }} style={{ background: '#0a0a0a', border: '1px solid #222', color: '#555', borderRadius: 8, padding: '10px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
                  </div>
                </div>
              )}
            </div>
            {!showAddKey && (
              <div style={{ padding: '10px 14px 24px', flexShrink: 0, borderTop: '1px solid #141414' }}>
                <button onClick={() => setShowAddKey(true)} style={{ width: '100%', background: 'linear-gradient(135deg,#ff8c00,#ff4400)', border: 'none', color: '#fff', borderRadius: 12, padding: '13px', fontSize: 13, fontWeight: 800, cursor: 'pointer', boxShadow: '0 4px 20px rgba(255,140,0,0.25)' }}>+ Naya API Key Add Karo</button>
              </div>
            )}
          </div>
        </>
...
function Topbar({ user, onSettings, onLogout, showBack, onBack }) {
  ...
  return (
    <div style={{ position: 'sticky', top: 0, zIndex: 10, background: 'rgba(8,8,8,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #161616', padding: '0 16px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {showBack ? (
          <button onClick={onBack} style={{ background: 'none', border: 'none', color: '#666', fontSize: 13, cursor: 'pointer', padding: '4px 0', fontWeight: 700 }}>← Back</button>
        ) : (
          <>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        <button onClick={onSettings} style={{ background: '#141414', border: '1px solid #222', color: '#555', borderRadius: 8, width: 32, height: 32, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>⚙️</button>
        <div ref={dropdownRef} style={{ position: 'relative' }}>
          <button onClick={() => setProfileOpen(p => !p)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
            {avatar
          ...
          {profileOpen && (
            <div style={{ position: 'absolute', top: 38, right: 0, background: '#0e0e0e', border: '1px solid #222', borderRadius: 14, minWidth: 210, zIndex: 50, boxShadow: '0 8px 32px rgba(0,0,0,0.7)', animation: 'fadeIn 0.15s ease', overflow: 'hidden' }}>
              ...
              <button onClick={() => { setProfileOpen(false); onSettings(); }} style={{ width: '100%', background: 'none', border: 'none', padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', color: '#aaa', fontSize: 12, fontWeight: 700, textAlign: 'left' }}>
                <span style={{ fontSize: 15 }}>⚙️</span> Settings
              </button>
              <div style={{ height: 1, background: '#161616', margin: '0 14px' }} />
              <button onClick={() => { setProfileOpen(false); onLogout(); }} style={{ width: '100%', background: 'none', border: 'none', padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', color: '#ff4444', fontSize: 12, fontWeight: 700, textAlign: 'left' }}>
